...
(function(root, factory) {
  ...
  function css(el, prop) {
    ...
    for (var p in prop)
      if (browser.fix(p)) el.style[browser.fix(p)] = prop[p];
    return el;
  }
  ...
  var browser = {
    ...
    createRule: function(name, props) {
      var rule = [name, '{'];
      for (var p in props)
        if (browser.fix(p, true)) rule.push(browser.fix(p, true) + ':' + props[p] + ';');
      rule.push('}');
      return rule.join(' ');
    },
    ...
    supports: function( /* prop1, prop2... */ ) {
      for (var i = 0, prop; prop = arguments[i]; i++)
        if (!browser.fix(prop)) return false;
      return true;
    },
    ...
    add: {
      ...
      'linear': function(name, type, tStart, tEnd, oStart, oEnd) {
        browser.createKeyframe(name, {
          ...
          '1': {
            'transform': 'translate' + tStart + 'px)',
            'opacity': oStart
          },
          '99': {
            'transform': 'translate' + tEnd + 'px)',
            'opacity': oEnd
          },
          '100': {
            'transform': 'translate' + tEnd[0] + '(0%)',
            'opacity': (type === 'out' ? '0' : oEnd),
            'visibility': 'hidden'
          }
        });
      },
      'cube': function(name, rStart, rEnd, tZ, oStart, oEnd) {
        browser.createKeyframe(name, {
          '0': {
            'transform': 'rotate' + rStart + '0deg) translateZ(' + tZ + 'px)',
            'opacity': oStart,
            'visibility': 'visible'
          },
          '100': {
            'transform': 'rotate' + rEnd + '0deg) translateZ(' + tZ + 'px)',
            'opacity': oEnd,
            'visibility': 'hidden'
          }
        });
  ...
  var callback = {
    ...
    meta: function(_, outs, ins, outdir, indir, outtrans, intrans) {
      return {
        'id': _.id,
        'in': {
          'el': slides.get(_, ins).el,
          'slidr': ins,
          'trans': intrans,
          'dir': indir
        },
        'out': {
          'el': slides.get(_, outs).el,
          'slidr': outs,
          'trans': outtrans,
          'dir': slides.opposite(outdir)
        }
      };
    },
  ...
  var slides = {
    ...
    get: function(_) {
      var args = [];
      for (var i = 1, a;
        (a = arguments[i++]) !== undefined; args.push(a)) {};
    ...
    validate: function(_, ids, trans, valid, prev, next) {
      ...
      for (var i = 0, current, newPrev, newNext, oldPrev, oldNext,
        ...
        if (slides.get(_, current)) {
          ...
          if ((oldNext && newNext && oldNext != newNext) || (oldPrev && newPrev && oldPrev != newPrev) || (prevPrev && prevPrev != current) || (newPrev && oldPrevTrans && oldPrevTrans != trans) || (newNext && oldNextTrans && oldNextTrans != trans)) {
            return false;
          }
        }
      }
  ...
  var controls = {
    ...
    create: function(_) {
      if (_.slidr && !_.controls) {
        _.controls = css(classname(createEl('aside', {
          'id': controls.cls.id(_)
        }), 'add', 'disabled'), {
          'opacity': '0',
  ...
  var breadcrumbs = {
    ...
    init: function(_) {
      if (_.slidr && !_.breadcrumbs) {
        _.breadcrumbs = css(classname(createEl('aside', {
          'id': breadcrumbs.cls.id(_)
        }), 'add', 'disabled'), {
          'opacity': '0',
          'filter': 'alpha(opacity=0)',
          'z-index': '0',
  ...
  var fx = {
    ...
    init: function(_, el, trans) {
      ...
      if (!s.initialized) {
        ...
        if (browser.isIE()) init = extend(init, {
          'display': 'none',
          'visibility': 'visible'
        });
        s.initialized = true;
  ...
  var nav = {
    ...
    mouse: {
      ...
      current: function() {
        ...
        for (var i = 0, l = nav.mouse.over.length, m = nav.mouse.over[i]; i < l; i++)
          if (contains(c, m)) c = m;
        return c;
      },
